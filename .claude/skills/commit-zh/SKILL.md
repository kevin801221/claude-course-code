---
name: commit-zh
description: Use whenever the user asks to commit changes, write a commit message, or run `git commit`. Produces Traditional Chinese commit messages in Kevin's personal style — concise, no "Claude Code" attribution, no emoji unless explicitly requested. Triggers on "commit", "幫我 commit", "寫 commit msg", "git commit", or when finalizing a series of edits.
---
